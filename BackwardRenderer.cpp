...
glm::vec3 BackwardRenderer::ComputeSampleColor(const IntersectionState& intersection, const Ray& fromCameraRay) const
{
    ...
    for (size_t i = 0; i < storedScene->GetTotalLights(); ++i) {
        ...
        for (size_t s = 0; s < sampleRays.size(); ++s) {
            ...
            while (true) {
                IntersectionState ip;
                result = storedScene->Trace(&sampleRays[s], &ip);
                //if (result == glm::vec3(0.f)) break;
                if (!result) break;
                if (!ip.intersectedPrimitive->GetParentMeshObject()->GetMaterial()->IsTransmissive()) {
                    inShadow = true;
                    break;
                }
                
                glm::vec3 pos = sampleRays[s].GetRayPosition(ip.intersectionT) + LARGE_EPSILON * fromCameraRay.GetRayDirection();
                sampleRays[s].SetRayPosition(pos);
                sampleRays[s].SetMaxT(sampleRays[s].GetMaxT() - intersection.intersectionT);
            }
            
            if (inShadow) continue;
            
            //glm::vec3 lightAttenuation = light->ComputeLightAttenuation(intersectionPoint) * result;
            float lightAttenuation = light->ComputeLightAttenuation(intersectionPoint);

            // Note that the material should compute the parts of the lighting equation too.
            const glm::vec3 brdfResponse = objectMaterial->ComputeBRDF(intersection, light->GetLightColor() * lightAttenuation, sampleRays[s], fromCameraRay, lightAttenuation);
